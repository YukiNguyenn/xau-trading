...
class BacktestManager:
    ...
    def _load_historical_data(self) -> Dict[str, pd.DataFrame]:
        ...
        # Add 200 days for technical indicators
        total_days = BACKTEST_DAYS + 200
        required_bars = {
            'M5': total_days * 24 * 12,  # 12 bars per hour * 24 hours * days
            'M15': total_days * 24 * 4,  # 4 bars per hour * 24 hours * days
            'H1': total_days * 24,       # 24 bars per day * days
            'H4': total_days * 6,        # 6 bars per day * days
            'D1': total_days             # 1 bar per day * days
        }
            ...
            try:
                ...
                if len(data[tf_name]) < required_bars[tf_name]:
                    self.logger.warning(
                        ...
                        f"need {required_bars[tf_name]} bars for {total_days} days"
                    )
    ...
    def _run_backtest(self, strategy_name: str, strategy: Any):
        ...
        try:
            ...
            for df_name, df in [('M15', m15_df), ('H4', h4_df), ('D1', d1_df), ('M5', m5_df), ('H1', h1_df)]:
                if df is not None:
                    ...
                    if len(df) == 0:
                        ...
                        return
                elif df_name in ['M5', 'H1'] and strategy_name == 'MultiTimeframeRSI':
            ...
            for i in range(len(m15_df)):
                ...
                if strategy_name == 'MultiTimeframeRSI' and (m5_data is None or len(m5_data) == 0):
                    self.logger.warning(f"Missing M5 data at {current_time}")
                    continue
